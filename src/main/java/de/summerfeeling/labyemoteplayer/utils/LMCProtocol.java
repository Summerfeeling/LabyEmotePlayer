...
public class LMCProtocol {
	...
	public static void sendLMCMessage(Player player, String key, JsonElement messageContent) {
		byte[] bytes = LMCProtocol.getBytesToSend(key, messageContent.toString());
		
		PacketDataSerializer serializer = new PacketDataSerializer(Unpooled.wrappedBuffer(bytes));
		PacketPlayOutCustomPayload payloadPacket = new PacketPlayOutCustomPayload("LMC", serializer);
